...
retry dnf install --assumeyes vim git wget curl rsync gnupg sysstat lsof pciutils usbutils psmisc
error

# Packages needed beyond a minimal install to build and run magma.
retry dnf install --assumeyes valgrind valgrind-devel texinfo autoconf automake libtool ncurses-devel gcc-c++ libstdc++-devel gcc cpp glibc-devel glibc-headers kernel-headers mpfr ppl perl perl-Module-Pluggable perl-Pod-Escapes perl-Pod-Simple perl-libs perl-version patch sysstat perl-Time-HiRes make cmake libarchive deltarpm net-tools python3-libdnf5
error

# Grab the required packages from the EPEL repo.
retry dnf install --assumeyes libbsd libbsd-devel
error
systemctl restart NetworkManager

# Boosts the available entropy which allows magma to start faster.
retry dnf install --assumeyes haveged
error
systemctl restart NetworkManager
...
retry dnf install --assumeyes wget git rsync perl-Git perl-Error
error
systemctl restart NetworkManager
